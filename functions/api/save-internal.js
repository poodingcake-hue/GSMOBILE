// Cloudflare Pages Functions: save-internal
// 브라우저에서 전달받은 편성 데이터를 GitHub REST API로 저장합니다.
// GITHUB_TOKEN 등 환경변수는 Cloudflare Pages 대시보드에서 설정해야 합니다.
...
export async function onRequest(context) {
  ...
  // Cloudflare Workers 환경변수는 context.env 객체에서 직접 꺼내어 사용합니다.
  const GITHUB_TOKEN = env.GITHUB_TOKEN;
  const GITHUB_OWNER = env.GITHUB_OWNER || 'poodingcake-hue';
  const GITHUB_REPO = env.GITHUB_REPO || 'GSMOBILE';

  if (!GITHUB_TOKEN) {
    return new Response(JSON.stringify({ error: 'GitHub 토큰이 서버에 설정되지 않았습니다. Cloudflare Pages 대시보드의 Environment Variables를 확인하세요.' }), {
      status: 500,
  ...
  const headers = {
    ...
    'User-Agent': 'Cloudflare-Pages-Worker', // GitHub API는 User-Agent를 필수로 요구합니다.
  };
  ...
  try {
    ...
    if (getRes.ok) {
      ...
      const decodedBase64 = atob(base64Clean);
      existingText = decodeURIComponent(
        decodedBase64.split('').map(c => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)).join('')
      );
